class _Form {
    ...
    setup() {
        console.debug('start form.setup()')
        this.defaultParams = this.#getFormValues()
        console.debug('デフォルト値', this.defaultParams)
        this.#setupDom()
        this.#setupEvent()
        console.debug('end form.setup()')
    }
    #setupDom() {
        this.#setupLabel()
    ...
    #setupAutocomplete() { for (let e of document.querySelectorAll('form select option[value]')) { e.textContent = e.value } }
    #setupDatalist() {
        this.#createDatalist('gitignore_template', this.#getLanguages())
        this.#createDatalist('license_template', this.#getLicenses())
    }
    #createDatalist(targetId, list) { // 0-4. 入力候補を作成・セットする
